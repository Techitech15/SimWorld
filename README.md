...
| 検討中の拡張                       | ドキュメント                                     | 状態               |
| ---------------------------------- | ------------------------------------------------ | ------------------ |
| 生物レイヤー（狩猟・牧畜・捕食者） | [docs/design-animals.md](docs/design-animals.md) | 設計案のみ、未実装 |

生物レイヤーは本編ロードマップに無い追加要素のため、フェーズ2（魔力インフラ）とは独立に着手できる
**フェーズ2.5** として位置づけている。捕食者を入れるために HP と死亡を最小構成で導入する点だけが
本編 9 章の「含まないもの」からの意図的な逸脱で、その範囲と限界は設計案の 1 章と 11 章に明記した。
