...
| `src/render/clouds.test.ts`    | 雲の影が純関数であること／時間で動くこと／alpha が上限を超えないこと／地図の外へ暴走しないこと（#15）／**画面占有率**が 60×60・120×120・180×180 のいずれでも下限を上回ること（修正前の5個の表を渡すと実際に割ることも確認）／重なった影の合成 alpha が夜（0.45）に届かないこと（#30） |
