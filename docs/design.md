...
| 項目 | 状態 | 内容 |
| --- | --- | --- |
| 研究台と `research` ジョブ | 未 | 仕事の列は 7 → 8。資源要求テックは魔導炉の燃料補給と同型の `haul` で成立。研究者の徴用は民兵と同型（列が有効な入植者が研究者） |
| 研究ツリー | 未 | 初期は4テック・2段。**既存の建築15種はロックしない**（解禁するのは新しいものだけ）。未解禁はメニューに灰色＋テック名で見せる |
| 職業 | 未 | 保存フィールドにしない。肩書きは最高スキルからの**導出**、職業プリセットは優先度表への**UI の一括設定**。専門化の機構はスキルが既にやっている |
