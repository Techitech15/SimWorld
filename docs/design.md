...
| 項目 | 状態 | 内容 |
| --- | --- | --- |
| 鉱脈の一元表（`VEIN_YIELD`） | 未 | 採掘産出の分岐2箇所（`execute.ts` / `mana.ts`）を1枚の表に。以降の鉱石追加は「表に1行」になる |
| 鉄（`iron` / `ironVein`） | 未 | 結晶より浅く多い2つ目の鉱石。道具・武器の上位素材の受け皿になる（フェーズ8で唯一材料が未決の防寒着は解決しない）。移行は v9→v10 と同型 |
| 家具5種 | 未 | 食卓・腰掛け・戸棚・安楽椅子・石像。効かせ先はすべて既存の系統（思考・睡眠回復・娯楽回復）で、新しいニーズは作らない。マイナスの思考は入れない |
| 建築メニューの階層化 | 未 | カテゴリ切替式（構造/家具/魔力/区域）。表示ボタン数がカテゴリ内最大数で頭打ちになる。定義を1枚の表に集約し、表示名の二重管理を消す |
