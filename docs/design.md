...
| [design-phase13-presentation.md](design-phase13-presentation.md) | **フェーズ13** 画面の整えと音の設計案（未着手） |
...
**`GameState` に増えるフィールドはゼロ**（すべて表示の性質。開閉・音量は `localStorage`）。

| 項目 | 状態 | 内容 |
| --- | --- | --- |
| ステータスの定位置化 | 未 | `.topbar__status` の幅100%折返しがズレの原因。ヘッダのサマリの隣の固定スロットへ |
| オーバーレイの折り畳み | 未 | 資源・ミニマップ・選択枠に既存 `Fold` を適用。アラートは畳みの対象外 |
| 選択枠のビューポート内表示 | 未 | `SelectionFrame` を盤面左下のオーバーレイへ（折り畳みあり）。サイドバーは植民地全体を見る場所に純化 |
| 効果音（実行時合成） | 未 | Web Audio で操作応答＋アラート級のみ。アセット0バイト・既定ミュート。**BGM と Diffusion Audio（生成モデル）は保留**（同設計案 5.2 の再検討条件つき） |
