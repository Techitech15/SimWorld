...
- 複数マップ・キャラバン・研究ツリー（交易は実装済み、キャラバンは非目標）
- 天候・気温管理（季節はあるが気温は無い）
...
| マップサイズの選択 | 未 | 寸法は state に入ったので表を1つ足すだけ |
...
| 項目 | 状態 | 内容 |
| --- | --- | --- |
| 補間の速さを歩調から導く | 未 | 描画は既に補間しているが `0.009 * deltaMs` の固定値。1倍速では 111 ms で1マスを渡り **289 ms 止まる**（これがカクつきの正体）。10倍速では逆に補間が追いつけない |
| 動物の補間を種ごとに | 未 | `ticksPerStep` が種ごとに 2〜4 と違うのに、表示側は全種同じ速さで寄せている |
| 状況で歩速を変える | 未 | `TICKS_PER_STEP` は全員一律の 2。荷・地形・体力・逃走の乗数にする。**このフェーズ唯一のセーブ追加**（`stepProgress`） |
| 昼夜のオーバーレイ | 未 | 時刻は既にある（`TICKS_PER_HOUR`・TopBar の `HH:MM`）が、地図には一切現れない。全面1枚の色で、タイル数に依らず一定コスト |
| 夜に魔力灯が明るい | 未 | `LAMP_RADIUS` と供給状態は計算済み。オーバーレイに穴を開けるだけ。**フェーズ2の見返りが初めて目に見える** |
| 夜に寝やすくする | 未 | 睡眠は現在 `needs.sleep >= SLEEP_THRESHOLD` だけで**時刻を見ていない**。昼夜を見せると真昼に寝る絵が壊れて見えるので、しきい値に時刻の乗数を掛ける |

### フェーズ8: 装備（服と武器） — 未着手

当初のロードマップに無い追加。設計案は [design-phase8-equipment.md](design-phase8-equipment.md)。部分ごとに前提が違う。当初は戦闘の未実装がボトルネックだったが、フェーズ4が入ったので**欠けている前提は気温だけ**になった。

| 部分 | 前提 | 内容 |
| --- | --- | --- |
| 作業具（斧・鶴嘴） | 作業台（[design-next.md](design-next.md) 提案3） | `WORK_TICKS` への乗数。特性・スキルと同じ形 |
| 狩猟具（弓・槍） | 同上 | 戦闘とは独立に意味を持つ —— `HUNT_RANGE = 5` が既にあり、射程8の弓は猪の突進（`BOAR_CHARGE_RANGE = 6`）の外に立てる |
| 防寒着 | 3 章の判断 | 気温は作らず、季節（冬）に直接効かせる。**材料が未決**（皮革は生物レイヤーの非目標） |
| 戦闘武器・防具 | 作業台＋フェーズ4（**実装済み**） | 襲撃と民兵が入ったので相手は既にいる。装備は既存の戦闘の数値に乗るだけにする |
