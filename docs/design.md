...
当初のロードマップに無い追加。設計案は [design-phase8-equipment.md](design-phase8-equipment.md)。**他と違い前提が揃っていない**ので、部分ごとに着手時期が分かれる。

| 部分 | 前提 | 内容 |
| --- | --- | --- |
| 作業具（斧・鶴嘴） | 作業台（[design-next.md](design-next.md) 提案4） | `WORK_TICKS` への乗数。特性・スキルと同じ形 |
| 狩猟具（弓・槍） | 同上 | **武器は戦闘を待たずに意味を持つ** —— `HUNT_RANGE = 5` が既にあり、射程8の弓は猪の突進（`BOAR_CHARGE_RANGE = 6`）の外に立てる |
| 防寒着 | 3 章の判断 | 気温は作らず、季節（冬）に直接効かせる。**材料が未決**（皮革は生物レイヤーの非目標） |
| 防具・戦闘武器 | **フェーズ4** | 入植者は反撃機構を持たないので、フェーズ4の設計時に改めて決める |
