...
| 雲の影（#15） | 済 | 純関数 `cloudsAt`（`src/render/clouds.ts` 新規）と、それをスプライトにする `syncClouds`（renderer）に分ける —— `daylight.ts` / `syncNight` と同じ分割で、決められる部分はすべてヘッドレスでテストできる側に置く。雲は全部が1つの風向きへ流れ、乱数は使わない。枚数は**地図の面積から導く**（`AREA_PER_CLOUD = 400`。120×120 で36枚、60×60 で9枚）——当初は5つの固定表だったが、地図が4倍になったとき表が追随せず画面占有率が18.1%まで落ちていた（#30、下段）。1枚ごとの位置・速度・大きさ・濃さは**添字から決定論的に導く**（位置は R2 低食い違い列、他は `variantAt` のハッシュ）ので、表を手で並べていたときと同じく乱数は無い。端の回り込みは地図の外へ余白を取ってその帯で alpha を0へ落としてから折り返す――座標が飛ぶ瞬間には必ず影が消えているので見た目の瞬間移動が起きない（テストで固定）。`state.speed === 0` の間は経過時間を進めない。レイヤは `colonistLayer` と `nightLayer` の間（影は建物も動物も暗くするが、夜のティントの下）。影のテクスチャは魔力灯のグロウと同じ DOM canvas の放射グラデーションを暗い色で作る。**風（草木の揺れ）は対象外**――スプライトのアニメフレーム追加になり、design-phase7-time.md 5章が非目標に挙げている性質のもの |
...
| 描画層の画面占有率（#30） | 済 | 「実装され、テストも全部緑で、**一度も画面に出ていない**」を捕まえる指標が無かった。`src/render/screenCoverage.ts`（新規）が、時間変化する円の集合を視界窓（16×14タイル）で走査して占有率を返す**純関数**。**雲に依存しない**ので、風（#23）が実装されたらそのまま使える。雲の枚数を面積から導くように直した結果、120×120 の占有率は **18.1% → 80.9%**。指標が問題を捕まえることは、**修正前の5個の表を通すと下限を割る**テストで別に示している（`tileVariant.test.ts` と同じ形）。60×60 / 120×120 / 180×180 の3寸法で下限を測るので、**また地図寸法が変わったときに気づける**。枚数が5→36に増えて初めて意味を持つ上限として、**重なった影の合成 alpha**（`1 - Π(1 - alphaᵢ)`）も固定した——2枚重なれば0.33、3枚で0.45＝`NIGHT_ALPHA` に届くため。実測の最悪値は 120×120 で0.306 |
