...
| 雲の影（#15） | 済 | 純関数 `cloudsAt`（`src/render/clouds.ts` 新規）と、それをスプライトにする `syncClouds`（renderer）に分ける —— `daylight.ts` / `syncNight` と同じ分割で、決められる部分はすべてヘッドレスでテストできる側に置く。雲は5つの固定表（位置・速度・大きさ・濃さ）で全部が1つの風向きへ流れ、乱数は使わない。**タイル数に依らず一定コスト**（スプライト5枚）。端の回り込みは地図の外へ余白を取ってその帯で alpha を0へ落としてから折り返す――座標が飛ぶ瞬間には必ず影が消えているので見た目の瞬間移動が起きない（テストで固定）。`state.speed === 0` の間は経過時間を進めない。レイヤは `colonistLayer` と `nightLayer` の間（影は建物も動物も暗くするが、夜のティントの下）。影のテクスチャは魔力灯のグロウと同じ DOM canvas の放射グラデーションを暗い色で作る。**風（草木の揺れ）は対象外**――スプライトのアニメフレーム追加になり、design-phase7-time.md 5章が非目標に挙げている性質のもの |
...
| 項目 | 状態 | 内容 |
| --- | --- | --- |
| 風が渡る明度の帯 | 済 | issue が挙げた3案（(a) スプライトへのアニメフレーム追加・(b) 描画時の変形・(c) 明度の帯を流す）のうち**(c) を採用**、(a)・(b) は不採用。雲の影（`src/render/clouds.ts`）が既に「動く帯を1枚重ねる」機構を持ち、風向きと速度を与えるだけで済むため——CLAUDE.md「新しい機構より、既存の数値を曲げる」に沿う判断。実装は純関数 `windAt`（`src/render/wind.ts`）+ `wind.test.ts` + `renderer.ts` の `syncWind`。`GUSTS` は4つの固定表で**乱数は使わない**——各ガストの位相は `tileVariant.ts` の `variantAt` をガストの座標（`x0`, `y0`）に当てて導出する。`WIND_STRENGTH_MAX = 0.08` は雲の影の `CLOUD_ALPHA_MAX`（0.18）の半分以下に置いた——2つの層が注意を奪い合わないため。**タイル数に依らない一定コスト**（スプライト4枚、雲の影と同じ性質）。`GameState` に増えたフィールドはゼロ、`schemaVersion` は不変。ポーズ中（`state.speed === 0`）は経過時間が進まず止まる。レイヤーは terrain の直後・building の手前（雲の影よりさらに下） |
...
**目視で見つかったこと**: 初回の実装は**一度も画面に現れなかった**。巨大な静止ガストを一時的に置いて確かめたところ描画機構は正しく、原因はパラメータだった —— 120×120 に 18×4 タイルの帯が4本、4.8〜6.5 タイル/秒では、視界（約16タイル）を横切るのが約3秒で残りの時間はどこにも無い。加えて明度上限 0.08 はそもそも見えない濃さだった。10本・26〜36タイル・1.8〜2.6タイル/秒・上限 0.22 へ調整した（[design-notes.md](design-notes.md)）。**10倍速での見え方の最終判断は未実施**。
