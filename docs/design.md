...
### フェーズ11: 世界（ワールドマップ・バイオーム・部族） — 段階A 実装済み
...
| 項目 | 状態 | 内容 |
| --- | --- | --- |
| バイオーム4種 | 済（段階A） | 草原（現行の生成そのもの）・深森・岩尾根・晶土。シナリオと同じ「レバーの束」で、地形生成の閾値と密度だけを曲げる。design-next 提案1(a)（鉱脈の下限保証）を全バイオームの下限として実装した。`GameState.biome` に直接保存（段階Bで `worldCell` からの導出キャッシュへ移行予定）。実測値は design-notes.md「バイオーム（フェーズ11 段階A）」 |
| ワールドマップ | 未 | 16×16セル。**`worldSeed` から導出し、保存しない**。`GameState` に入るのは選んだセル座標だけ。「おまかせ」で従来の1クリック開始も残す |
| 部族3つ | 未 | 灯持ち（交易の出どころ）・森歩き（移住の出どころ）・渇き衆（襲撃の出どころ）。実体を持たず、選んだセルからの距離が既存の頻度・規模への乗数になる |
